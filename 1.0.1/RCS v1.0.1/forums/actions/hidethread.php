...
if($thread->checkExistence($id) && $thread->canView($id, $username, $rank) && $rank > 2)
{
    if($_GET['type'] == 2)
    {
        $thread->coverThread($id, $rank); $base->appendToFile('../logs.txt', array($username.' hid the thread '. $id));
    }
    else
    {
        $thread->hideThread($id, $rank); $base->appendToFile('../logs.txt', array($username.' hid the thread '. $id));
    }
...
You have successfully <?php echo ($_GET['type'] == 2) ? 'covered/uncovered' : 'hid/unhid'; ?> the thread!
<br/>
<br/>
