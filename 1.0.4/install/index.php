...
if(isset($_SESSION['step1'])){
    ...
    if(isset($_POST['host']) && isset($_POST['user']) && isset($_POST['pass']) && isset($_POST['db'])){
        ...
        if($db){
            ...
            $path = substr($url, 0, strpos($url, '/install/')+1);
            $path = str_replace('install/', null, $path);
            
            if(preg_match('#\/(.+?)\/#', $path)){
                $path = $_SERVER['HTTP_HOST'].$path;
            }else{
                $path = str_replace('/', null, $_SERVER['HTTP_HOST']).'/';
            }
            ...
            //write data
            $old = array('{host}', '{user}', '{name}', '{pass}', '{path}');
            $new = array($_POST['host'], $_POST['user'], $_POST['db'], $_POST['pass'], $path);
            ...
            ");
            $db->query("INSERT INTO `users` VALUES(1, 'Report', '43e55dc74d83bad897cb49b784c49e418304f8544b81ed015f77617dbe80c3edf5f6e702b25f721d82a962a10f6764897dd7b24b5e3cf9bd1e2a6b43e', '13-18', '225', 'fdsfdsf@gmail.com', 'Jun/22/2012', 4, '209.82.161.101', 1348344761, 1340362924, '". hash(sha512, time()) ."', 0, 0, 1, 0, 0, '');");
