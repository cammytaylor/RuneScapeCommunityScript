...
class user
{
    ...
    public function __construct(database $database)
    {
        ...
        if(isset($_COOKIE['user']))
        {
            ...
            if($this->database->getRowCount() == 0 || $d[0]['acc_status'] == 0 || $results >= 1 || ($d[0]['lastip'] != $_SERVER['REMOTE_ADDR']))
            {
    ...
    public function isDisabled($username)
    {
        ...
        $ban_query = $this->database->processQuery("SELECT `acc_status` FROM `users` WHERE `username` = ?", array($username), true);
        $banned = ($ban_query[0]['acc_status'] == 0) ? true : false;
        
        if($this->getRank($username) == 0 || $x >= 1 || $banned)
            return true;
        else
            return false;
    }
